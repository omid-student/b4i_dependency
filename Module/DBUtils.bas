...
Public Sub CopyDBFromAssets (FileName As String) As String
	Dim TargetDir As String = File.DirDocuments
...
Public Sub CreateTable(SQL As SQL, TableName As String, FieldsAndTypes As Map, PrimaryKey As String)
	...
	sb.Append("(")
	Dim i As Int = 0
	For Each field As String In FieldsAndTypes.Keys
		Dim ftype As String = FieldsAndTypes.Get(field)
...
Public Sub UpdateRecord(SQL As SQL, TableName As String, Field As String, NewValue As Object, _
	...
	args.Add(NewValue)
	Dim i As Int
	For Each key As String In WhereFieldEquals.Keys
		If i > 0 Then sb.Append(" AND ")
		sb.Append("[").Append(key).Append("] = ?")
		args.Add(WhereFieldEquals.Get(key))
		i = i + 1
	Next
...
'Returns an uninitialized map if no results found.
Public Sub ExecuteMap(SQL As SQL, Query As String, StringArgs() As String) As Map
	...
	For i = 0 To rs.ColumnCount - 1
		res.Put(rs.GetColumnName(i).ToLowerCase, rs.GetString2(i))
	Next
	rs.Close
	Return res
End Sub

Public Sub ExecuteList(SQL As SQL, Query As String, StringArgs() As String, _
	Limit As Int) As List
	Dim table As List = ExecuteMemoryTable(SQL, Query, StringArgs, Limit)
	Dim items As List
	items.Initialize
	For Each row() As String In table
		If Limit > 0 AND items.Size >= Limit Then Exit
		items.Add(row(0))
	Next
	Return items
End Sub
...
'Usage example:<code> (don't forget to add a reference to the JSON library)
...
Public Sub ExecuteJSON (SQL As SQL, Query As String, StringArgs() As String, Limit As Int, DBTypes As List) As Map
	Dim table As List
	Dim cur As ResultSet = SQL.ExecQuery2(Query, StringArgs)
...
Public Sub ExecuteHtml(SQL As SQL, Query As String, StringArgs() As String, Limit As Int, Clickable As Boolean) As String
	...
	Do While cur.NextRow
		...
		sb.Append("</tr>").Append(CRLF)
		row = row + 1
		If Limit > 0 AND row >= Limit Then Exit
	Loop
	cur.Close
...
Public Sub DeleteRecord(SQL As SQL, TableName As String, WhereFieldEquals As Map)
    ...
    args.Initialize
	Dim i As Int = 0
    For Each field As String In WhereFieldEquals.Keys
        If i > 0 Then sb.Append(" AND ")
        sb.Append("[").Append(field).Append("] = ?")
        args.Add(WhereFieldEquals.Get(field))
		i = i + 1
    Next
